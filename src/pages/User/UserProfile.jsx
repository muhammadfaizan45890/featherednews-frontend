...
const SkeletonProfile = () => (
  <div className="min-h-screen bg-white px-3 sm:px-4 md:px-6">
    <div className="max-w-6xl mx-auto pt-4 sm:pt-10 md:pt-14">
      <div className="h-20 sm:h-32 md:h-44 lg:h-52 rounded-2xl bg-gray-200" />
      <div className="px-2 sm:px-4 -mt-8 sm:-mt-12 flex items-end gap-4">
        <div className="w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-gray-300" />
        <div className="flex-1 pb-2 space-y-2">
          <div className="h-5 sm:h-7 w-40 sm:w-56 rounded bg-gray-300" />
          <div className="h-3 sm:h-4 w-24 sm:w-32 rounded bg-gray-300" />
        </div>
      </div>
    </div>
  </div>
...
const EditableField = ({ icon: Icon, label, value, name, onChange, isEditing, type = "text", placeholder }) => (
  <div className="group flex items-start gap-3 py-3 sm:py-3.5 border-b border-gray-200 last:border-0">
    <div className="mt-0.5 w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gray-100 text-gray-500 flex items-center justify-center flex-shrink-0">
      <Icon size={14} className="sm:w-4 sm:h-4" />
    </div>
    <div className="flex-1 min-w-0">
      <p className="font-meta text-[9px] sm:text-[10px] text-gray-400 uppercase tracking-wider">{label}</p>
      {isEditing ? (
        type === "textarea" ? (
          <textarea
            name={name}
            ...
            onChange={onChange}
            rows={2}
            className="ink-focus w-full mt-1 bg-transparent border-b-2 border-gray-300 focus:border-black outline-none py-0.5 text-gray-900 text-sm sm:text-base resize-none"
          ...
          <input
            ...
            value={value}
            onChange={onChange}
            className="ink-focus w-full mt-1 bg-transparent border-b-2 border-gray-300 focus:border-black outline-none py-0.5 text-gray-900 text-sm sm:text-base"
            placeholder={placeholder || `Enter ${label}`}
          />
        )
      ) : (
        <p className="font-medium text-gray-900 text-sm sm:text-base break-all mt-0.5">
...
const SocialCube = ({ icon: Icon, href, label, color }) => {
  ...
  return (
    <a
      ...
      className={`ink-focus w-11 h-11 sm:w-12 sm:h-12 rounded-lg border-2 border-gray-200 flex items-center justify-center text-gray-600 hover:text-white hover:border-transparent ${color}`}
      aria-label={label}
      title={label}
    >
...
const EmptyState = ({ icon: Icon, title, subtitle }) => (
  <div className="flex flex-col items-center justify-center text-center py-12 sm:py-16 px-4">
    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gray-100 flex items-center justify-center mb-3 sm:mb-4">
      <Icon size={22} className="sm:w-6 sm:h-6 text-gray-400" />
    </div>
    <p className="font-display text-base sm:text-lg font-semibold text-gray-800">{title}</p>
    <p className="text-xs sm:text-sm text-gray-500 mt-1 max-w-xs">{subtitle}</p>
  </div>
);
...
const UserProfile = () => {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-white p-5 flex items-center justify-center px-4">
        <div className="text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-3 sm:mb-4">
            <User size={24} className="sm:w-8 sm:h-8 text-red-500" />
          </div>
          <h2 className="font-display text-xl sm:text-2xl font-bold text-gray-900">User Not Found</h2>
          <p className="text-sm sm:text-base text-gray-600 mt-2">Your session may have expired.</p>
          <button
            onClick={() => navigate("/login")}
            className="ink-focus mt-4 sm:mt-6 px-4 sm:px-6 py-1.5 sm:py-2 bg-black text-white rounded-xl font-semibold text-sm sm:text-base"
          >
            Go to Login
          </button>
        </div>
        ...
        <div className="relative">
          <div className="relative px-2 sm:px-4 flex flex-wrap items-end gap-3 sm:gap-5 md:gap-6">
            <div className="relative group flex-shrink-0">
              <div
                className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 rounded-full overflow-hidden flex items-center justify-center bg-white"
              ...
              {isEditing && (
                <>
                  <button
                    ...
                    className="ink-focus absolute bottom-0 right-0 sm:bottom-1 sm:right-1 bg-black text-white rounded-full p-1.5 sm:p-2"
            ...
            <div className="flex-1 min-w-[160px] pb-2 sm:pb-3">
              {isEditing ? (
                <input
                  ...
                  className="ink-focus font-display text-xl sm:text-2xl md:text-3xl font-semibold bg-transparent border-b-2 border-gray-300 focus:border-black outline-none w-full text-gray-900"
                  placeholder="Full name"
                />
              ) : (
                <h1 className="font-display text-xl sm:text-2xl md:text-3xl font-semibold text-gray-900 truncate">
                  {user.fullname || user.username}
                </h1>
              )}
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 font-meta text-[10px] sm:text-xs text-gray-500">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                {user.role && (
                  <span className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-semibold font-meta bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full uppercase tracking-wide">
                    <Award size={10} />
                    {user.role}
                  </span>
                )}
                {user.isVerified && (
                  <span className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-semibold font-meta bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full uppercase tracking-wide">
            ...
            <div className="flex flex-wrap gap-2 pb-2 sm:pb-3">
              {!isEditing ? (
                <button
                  ...
                  className="ink-focus flex items-center gap-1.5 px-3.5 sm:px-4 py-2 bg-black text-white rounded-xl text-xs sm:text-sm font-semibold"
                >
                ...
                <>
                  <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className="ink-focus flex items-center gap-1.5 px-3.5 sm:px-4 py-2 bg-black text-white rounded-xl text-xs sm:text-sm font-semibold disabled:opacity-60"
                  >
                  ...
                  <button
                    ...
                    className="ink-focus flex items-center gap-1.5 px-3.5 sm:px-4 py-2 bg-gray-200 text-gray-700 rounded-xl text-xs sm:text-sm font-semibold hover:bg-gray-300"
                  >
                    <X size={14} />
                    Cancel
                  </button>
          ...
          <div className="relative px-2 sm:px-4 mt-2 sm:mt-3 flex flex-wrap items-center gap-3 sm:gap-4">
            <span className="font-meta text-[10px] sm:text-xs text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
            ...
            {!hasSocials && !isEditing && (
              <span className="text-xs sm:text-sm text-gray-400 italic">No links added yet.</span>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 sm:mt-8 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 sm:gap-6">
          ...
          <aside className="space-y-4 sm:space-y-6 lg:sticky lg:top-6 lg:self-start">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5">
              <h3 className="font-meta text-[10px] text-gray-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
              ...
              {isEditing ? (
                <textarea
                  ...
                  onChange={handleInputChange}
                  rows={4}
                  className="ink-focus w-full bg-transparent border-b-2 border-gray-300 focus:border-black outline-none py-1 text-gray-900 text-sm resize-none"
                  placeholder="Tell readers about yourself"
                />
              ) : (
                <p className="font-display text-[15px] sm:text-base text-gray-800 leading-relaxed">
                  {user.bio || <span className="font-sans text-gray-400 italic text-sm">No bio yet — tell readers who you are.</span>}
                </p>
              )}

              {user.occupation && !isEditing && (
                <p className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-500 mt-3 pt-3 border-t border-gray-100">
              ...
              {user.website && !isEditing && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ink-focus flex items-center gap-1.5 text-xs sm:text-sm text-gray-900 mt-2 font-medium hover:underline underline-offset-2"
                >
          ...
          <main className="min-w-0">
            ...
            {activeTab === "overview" && (
              <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6">
                <h3 className="font-meta text-[10px] text-gray-400 uppercase tracking-wider mb-1">
          ...
          <button
            ...
            className="ink-focus w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 font-medium text-sm"
          >
            <LogOut size={15} />
            Log out
          </button>
